...
class Trainer:
    ...
    def __init__(
        ...
        self.ob = SlacObservation(env.observation_space.shape, env.ometer_space.shape, env.tgt_state_space.shape,
                                  env.action_space.shape, num_sequences)
        self.ob_test = SlacObservation(env.observation_space.shape, env.ometer_space.shape, env.tgt_state_space.shape,
                                       env.action_space.shape, num_sequences)
    ...
    def evaluate(self, step_env):
        ...
        for i in range(self.num_eval_episodes):
            ...
            while not done:
                ...
                if steps_until_dump_obs == 0:
                    ...
                    reconstruction = \
                    sample_reproduction(self.algo.latent, self.algo.device, self.ob_test.state, self.ob_test.ometer,
                                        np.array([self.ob_test._action]))[0][-1] * 255
        ...
        print(
            f"Steps: {step_env:<6}   " f"Return: {mean_reward_return:<5.1f} " f"CostRet: {mean_cost_return:<5.1f}   " f"Time: {self.time}")
