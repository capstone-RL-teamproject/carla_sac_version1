...
# gym.spaces 대신 gymnasium.spaces 사용
# import gymnasium
# from gym.spaces import Box, Tuple, Dict, Discrete
from gymnasium.spaces import Box, Dict, Tuple, Discrete
import ray
...
class DisplayManager:
        ...
        try:
            if display_sensor:
                self.display = pygame.display.set_mode(
                    window_size, pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.SHOWN
                )
            else:
                self.display = pygame.display.set_mode(
                    window_size, pygame.HWSURFACE | pygame.DOUBLEBUF | pygame.HIDDEN
                )
    ...
    def render(self):
        ...
        """
        self.display.blit(
            self.hud.surface, self.get_display_offset(self.hud.display_pos)
        )
        self.display.blit(
            self.hud.surface_global,
            self.get_display_offset(self.hud.display_pos_global),
        )
...
class CarlaRlEnv(gym.Env):
    def __init__(self, params):
        # suepr 추가
        # super(CarlaRlEnv, self).__init__()
        # resource from outside
        # parse parameters
        ...
        if self.sync:
            traffic_manager = self.client.get_trafficmanager(
                8000 + self.worker_rank * 100
            )  # actor,traffic manage
            settings = self.world.get_settings()
        ...
        self.display_manager = DisplayManager(
            self.grid_size, self.window_resolution, self.display_sensor
        )
        ...
        self.action_space = Tuple(
            (
                Box(np.array([0.0, 0.0, -1.0]), 1.0, shape=(3,), dtype=np.float32),
                Discrete(2),
            )
        )
        ...
        #     'front_camera': Box(low=0, high=255, shape=(self.display_size[0], self.display_size[1], 3), dtype=np.uint8),
        #     'left_camera': Box(low=0, high=255, shape=(self.display_size[0], self.display_size[1], 3), dtype=np.uint8),
        #     'right_camera': Box(low=0, high=255, shape=(self.display_size[0], self.display_size[1], 3), dtype=np.uint8),
        #     'top_camera': Box(low=0, high=255, shape=(self.display_size[0], self.display_size[1], 3), dtype=np.uint8),
        #     'lidar_image': Box(low=0, high=255, shape=(self.display_size[0], self.display_size[1], 3), dtype=np.uint8),
        #     'radar_image': Box(low=0, high=255, shape=(self.display_size[0], self.display_size[1], 3), dtype=np.uint8),
        #     'hud': Box(low=0, high=255, shape=(self.display_size[0], self.display_size[1], 3), dtype=np.uint8),
        #     'trgt_pos': Box(low=-np.inf, high=np.inf, shape=(3,), dtype=np.float32),
        #     'wp_hrz': Box(low=-np.inf, high=np.inf, shape=(40, 2), dtype=np.float32)
        # }
        ...
        self.observation_space = Dict(
            {
                "top_camera": Box(
                    low=0,
                    high=255,
                    shape=(self.display_size[0], self.display_size[1], 3),
                    dtype=np.uint8,
                )
            }
        )
        ...
        self.tgt_state_space = Box(0, 255, (3, self.height, self.width), dtype=np.uint8)
    ...
    # reset, step concate 수정 & channel 변경
    def step(self, action):
        ...
        src_img = np.transpose(img_np_resized, [2, 1, 0])

        return src_img, re[1], re[2], re[3]
    ...
    def carla_step(self, action):
        ...
        act = carla.VehicleControl(
            throttle=float(acc), steer=float(trn), brake=float(brk), reverse=bool(rvs)
        )
        ...
        observation = {
            "top_camera": (
                self.top_camera.measure_data
                if self.top_camera is not None
                else np.zeros(img_size)
            ),
        }
    ...
    def update_waypoints_and_horizon(self):
        ...
        num_checked_waypoints = min(
            10, len(self.waypoints)
        )  # 최대 10개의 경로만 계산한다
        dis = 10000.0
        idx = 1000
        ...
        for n_c_wp in range(num_checked_waypoints):
            if (
                current_location.distance(self.waypoints[n_c_wp][0].transform.location)
                < dis
            ):
                dis = current_location.distance(
                    self.waypoints[n_c_wp][0].transform.location
                )
        ...
        waypoints_horizon = (
            []
        )  # 차량이 따라야할 미래 경로 나타내는 리스트 최대 40개의 경로점만 미리 계산
        if len(self.waypoints) > 40:
            wpsh = self.waypoints[:40]
        else:
            wpsh = self.waypoints + [self.waypoints[-1]] * (40 - len(self.waypoints))
        for wph in wpsh:
            waypoints_horizon.append(
                np.array([wph[0].transform.location.x, wph[0].transform.location.y])
            )
    ...
    def deal_with_reward_and_done(self):
        ...
        self.cost = 0.0
        current_location, wp = (
            self.update_waypoints_and_horizon()
        )  # 현재 차량 위치, 현재 위치에서 가장가까운 마지막 waypoint
        """
        ...
        if self.lane_invasion.measure_data:
            if (
                self.lane_invasion.measure_data == "Broken"
                or self.lane_invasion.measure_data == "BrokenSolid"
                or self.lane_invasion.measure_data == "BrokenBroken"
            ):
        ...
        if current_location.distance(current_wp.transform.location) > (
            current_wp.lane_width / 2.0 + 0.2
        ):
            self.done = True
            lane_invasion_cost = 1.0
        ...
        lat_err, omg = cal_error_2D(
            wp, current_location
        )  # 측면으로 얼마나 떨어져 있는지, 목표 경로점 방향
        ...
        v_long = np.dot(
            np.array([current_velocity.x, current_velocity.y]), omg
        )  # 현재 차량 속도와 목표경로점 방향이 얼마나 일치하는지 본다
    ...
    def create_all_actors(self):
        ...
        )  # 지도에서 목표지점 위치 알아내서 이곳에 넘겨야함
        self.target_pos.set_transform(random.choice(self.spawn_points))

        # create ego vehicle
        ego_vehicle_bp = random.choice(
            [
                bp
                for bp in self.world.get_blueprint_library().filter(self.ego_filter)
        ...
        while self.ego_vehicle is None:
            self.ego_vehicle = self.world.try_spawn_actor(
                ego_vehicle_bp, random.choice(self.spawn_points)
            )
            time.sleep(0.1)
        ...
        self.waypoints = self.route_planner_global.trace_route(
            self.ego_vehicle.get_location(), self.target_pos.transform.location
        )  # planner따른 경로 설정
        ...
        if "front_rgb" in self.sensors_to_amount:
            self.front_camera = SensorManager(
                ...
                "RGBCamera",
                carla.Transform(
                    carla.Location(x=0, z=bbe_z + 1.4), carla.Rotation(yaw=+00)
                ),
                self.ego_vehicle,
                {},
                self.display_size,
        ...
        if "left_rgb" in self.sensors_to_amount:
            self.left_camera = SensorManager(
                self.world,
                "RGBCamera",
                carla.Transform(
                    carla.Location(x=0, z=bbe_z + 1.4), carla.Rotation(yaw=-90)
                ),
                self.ego_vehicle,
                {},
                self.display_size,
        ...
        if "right_rgb" in self.sensors_to_amount:
            self.right_camera = SensorManager(
                self.world,
                "RGBCamera",
                carla.Transform(
                    carla.Location(x=0, z=bbe_z + 1.4), carla.Rotation(yaw=+90)
        ...
        if "radar" in self.sensors_to_amount:
            ...
            self.radar = SensorManager(
                ...
                self.ego_vehicle,
                {"horizontal_fov": "60", "vertical_fov": "30", "range": "20.0"},
                self.display_size,
                [1, 2],
            )
        ...
        self.collision = SensorManager(
            self.world, "Collision", carla.Transform(), self.ego_vehicle, {}, None, None
        )

        self.sensor_list.append(self.collision)

        self.lane_invasion = SensorManager(
            self.world,
            "Lane_invasion",
            carla.Transform(),
            self.ego_vehicle,
        ...
        vehicle_bps_4wheel = []
        vehicle_bps_4wheel = vehicle_bps_4wheel + [
            x for x in vehicle_bps if int(x.get_attribute("number_of_wheels")) == 4
        ]
        ...
        for _ in range(self.num_vehicles):
            ...
            while vehicle_tmp_ref is None:
                vehicle_tmp_ref = self.world.try_spawn_actor(
                    random.choice(vehicle_bps_4wheel), random.choice(self.spawn_points)
                )
        ...
        for _ in range(self.num_pedestrians):
            ...
            while pedestrian_tmp_ref is None:
                ...
                pedestrian_tmp_ref = self.world.try_spawn_actor(
                    random.choice(pedestrian_bps), pedestrian_spawn_transform
                )

            pedestrian_controller_bp = self.world.get_blueprint_library().find(
                "controller.ai.walker"
            )
            pedestrian_controller_actor = self.world.spawn_actor(
                pedestrian_controller_bp, carla.Transform(), pedestrian_tmp_ref
            )
